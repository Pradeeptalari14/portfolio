  ...
  setupCompilerTriggers(triggerCompileAll);
    ...
    function compileMermaidFlow() {
      ...
      if (purpose === 'aws_finops') {
        ...
        flow += `    Start([Start Cost Sweep]) --> Init[Initialize boto3 EC2 Client]\n`;
        if (auditEbs) {
          flow += `    Init --> ScanEBS[Scan for unattached EBS volumes]\n`;
          flow += `    ScanEBS --> LoopEBS{For each Volume}\n`;
          flow += `    LoopEBS -->|Unattached| CheckDryEBS{Dry Run?}\n`;
          flow += `    CheckDryEBS -->|No| DelVol[Delete Volume via EC2 API]\n`;
          flow += `    CheckDryEBS -->|Yes| LogEBS[Log volume ID & size]\n`;
          if (auditEip) {
            flow += `    DelVol --> ScanEIP[Scan for unassociated EIPs]\n`;
            flow += `    LogEBS --> ScanEIP[Scan for unassociated EIPs]\n`;
          } else {
            flow += `    DelVol --> Finish([Finish Cost Sweep])\n`;
            flow += `    LogEBS --> Finish\n`;
          }
        } else {
          if (auditEip) {
            flow += `    Init --> ScanEIP[Scan for unassociated EIPs]\n`;
          } else {
            flow += `    Init --> Finish([Finish Cost Sweep])\n`;
          }
        }

        if (auditEip) {
          flow += `    ScanEIP --> LoopEIP{For each EIP}\n`;
          flow += `    LoopEIP -->|Unused| CheckDryEIP{Dry Run?}\n`;
          flow += `    CheckDryEIP -->|No| RelAddr[Release Address via EC2 API]\n`;
          flow += `    CheckDryEIP -->|Yes| LogEIP[Log unused public IP]\n`;
          flow += `    RelAddr --> Finish([Finish Cost Sweep])\n`;
          flow += `    LogEIP --> Finish\n`;
        }
      } else if (purpose === 'k8s_monitor') {
        const incluster = $('k8s_incluster').checked;
        ...
        flow += `    Start([Start Pod Monitor]) --> LoadConfig{In-Cluster Config?}\n`;
        if (incluster) {
          flow += `    LoadConfig -->|Yes| LoadIn[load_incluster_config]\n`;
          flow += `    LoadIn --> Init[Initialize CoreV1Api]\n`;
        } else {
          flow += `    LoadConfig -->|No| LoadKube[load_kube_config]\n`;
          flow += `    LoadKube --> Init[Initialize CoreV1Api]\n`;
        }
        flow += `    Init --> Scan[List target pods in namespace]\n`;
        flow += `    Scan --> Loop[Loop pods container status]\n`;
        flow += `    Loop --> CheckRestarts{Restarts >= Threshold?}\n`;
        flow += `    CheckRestarts -->|Yes| AlertRestart[Dispatch restart alert webhook]\n`;
        if (oom) {
          flow += `    CheckRestarts -->|No| CheckState{Check State reason?}\n`;
          flow += `    AlertRestart --> CheckState\n`;
          flow += `    CheckState -->|CrashLoopBackOff| AlertState[Dispatch state alert webhook]\n`;
          flow += `    CheckState -->|OOMKilled| AlertOOM[Dispatch OOMKilled alert webhook]\n`;
          flow += `    CheckState -->|Normal| NextPod[Move to next pod]\n`;
          flow += `    AlertState --> NextPod\n`;
          flow += `    AlertOOM --> NextPod\n`;
          flow += `    NextPod --> Finish([Finish pod scan])\n`;
        } else {
          flow += `    CheckRestarts -->|No| NextPod[Move to next pod]\n`;
          flow += `    AlertRestart --> NextPod\n`;
          flow += `    NextPod --> Finish([Finish pod scan])\n`;
        }
      } else if (purpose === 'ssl_expiry') {
        flow += `    Start([Start SSL Expiry Check]) --> Loop[For each domain in list]\n`;
        flow += `    Loop --> Connect[Establish socket connection on port]\n`;
        flow += `    Connect --> Wrap[Wrap socket with TLS context]\n`;
        flow += `    Wrap --> GetCert[Retrieve TLS Certificate details]\n`;
        flow += `    GetCert --> ParseDate[Parse expiry date string]\n`;
        flow += `    ParseDate --> CalcRemaining[Calculate remaining days]\n`;
        flow += `    CalcRemaining --> CheckExpiry{Days < Warning threshold?}\n`;
        flow += `    CheckExpiry -->|Yes| Alert[Dispatch SSL expiry webhook alert]\n`;
        flow += `    CheckExpiry -->|No| Log[Log cert health status]\n`;
        flow += `    Alert --> LoopNext[Next domain]\n`;
        flow += `    Log --> LoopNext\n`;
        flow += `    LoopNext --> Finish([Finish SSL audit])\n`;
      } else if (purpose === 'api_ping') {
        flow += `    Start([Start Latency Telemetry]) --> ThreadPool[Initialize ThreadPoolExecutor]\n`;
        flow += `    ThreadPool --> WorkerLoop[Submit concurrent GET requests]\n`;
        flow += `    WorkerLoop --> Measure[Measure latency & status code]\n`;
        flow += `    Measure --> Aggregate[Collect latency metrics]\n`;
        flow += `    Aggregate --> Stats[Calculate Average, P95 & P99]\n`;
        flow += `    Stats --> CSV[Export telemetry to latency_results.csv]\n`;
        flow += `    CSV --> AlertLatency{P95 Latency > 2000ms?}\n`;
        flow += `    AlertLatency -->|Yes| Alert[Dispatch high latency webhook alert]\n`;
        flow += `    AlertLatency -->|No| Finish([Finish telemetry test])\n`;
        flow += `    Alert --> Finish\n`;
      } else if (purpose === 'sys_monitor') {
        const daemonMode = $('monitor_daemon').checked;

        flow += `    Start([Start System Monitor]) --> Loop{Daemon Mode?}\n`;
        if (daemonMode) {
          flow += `    Loop -->|Yes| QueryCPU[Query CPU utilization %]\n`;
        } else {
          flow += `    Loop -->|No| QueryCPU[Query CPU utilization %]\n`;
        }
        flow += `    QueryCPU --> CheckCPU{CPU > Threshold?}\n`;
        flow += `    CheckCPU -->|Yes| AlertCPU[Dispatch CPU alert webhook]\n`;
        flow += `    CheckCPU -->|No| QueryRAM[Query virtual memory RAM %]\n`;
        flow += `    AlertCPU --> QueryRAM\n`;
        flow += `    QueryRAM --> CheckRAM{RAM > Threshold?}\n`;
        flow += `    CheckRAM -->|Yes| AlertRAM[Dispatch RAM alert webhook]\n`;
        flow += `    CheckRAM -->|No| QueryDisk[Query root disk partition %]\n`;
        flow += `    AlertRAM --> QueryDisk\n`;
        flow += `    QueryDisk --> CheckDisk{Disk > Threshold?}\n`;
        flow += `    CheckDisk -->|Yes| AlertDisk[Dispatch Disk alert webhook]\n`;
        flow += `    CheckDisk -->|No| EndCheck{Daemon loop?}\n`;
        flow += `    AlertDisk --> EndCheck\n`;
        if (daemonMode) {
          flow += `    EndCheck -->|Yes| Sleep[Sleep interval seconds]\n`;
          flow += `    Sleep --> QueryCPU\n`;
        } else {
          flow += `    EndCheck -->|No| Finish([Finish monitor check])\n`;
        }
      } else if (purpose === 'log_analyzer') {
        flow += `    Start([Start Log Analyzer]) --> FileExists{Log file exists?}\n`;
        flow += `    FileExists -->|No| AlertMissing[Log error & alert file not found]\n`;
        flow += `    FileExists -->|Yes| OpenFile[Open log file in read-only stream]\n`;
        flow += `    OpenFile --> Loop[Loop through each log line]\n`;
        flow += `    Loop --> MatchRegex{Matches CLF regex?}\n`;
        flow += `    MatchRegex -->|Yes| Extract[Extract IP, request, status]\n`;
        flow += `    MatchRegex -->|No| Skip[Skip line]\n`;
        flow += `    Extract --> CountStatus[Increment status frequency counter]\n`;
        flow += `    CountStatus --> FilterStatus{Matches status filter?}\n`;
        flow += `    FilterStatus -->|Yes| IncrementError[Increment error counter & track IP]\n`;
        flow += `    FilterStatus -->|No| LoopNext[Read next line]\n`;
        flow += `    IncrementError --> LoopNext\n`;
        flow += `    Skip --> LoopNext\n`;
        flow += `    LoopNext --> EOF{End of file?}\n`;
        flow += `    EOF -->|No| Loop\n`;
        flow += `    EOF -->|Yes| PrintStats[Print HTTP status frequency tables]\n`;
        flow += `    PrintStats --> CheckErrors{Any errors found?}\n`;
        flow += `    CheckErrors -->|Yes| AlertOutliers[Dispatch alert with top IP outliers]\n`;
        flow += `    CheckErrors -->|No| Finish([Finish log analysis])\n`;
        flow += `    AlertOutliers --> Finish\n`;
      } else if (purpose === 'disk_cleanup') {
        const dry = $('cleanup_dry_run').checked;

        flow += `    Start([Start Disk Cleanup]) --> TargetExists{Target folder exists?}\n`;
        flow += `    TargetExists -->|No| AlertMissing[Log error & alert target not found]\n`;
        flow += `    TargetExists -->|Yes| WalkDir[Recursively walk target directory tree]\n`;
        flow += `    WalkDir --> LoopFiles[For each file found]\n`;
        flow += `    LoopFiles --> CheckExt{Extension in target list?}\n`;
        flow += `    CheckExt -->|Yes| GetStats[Get last modified mtime & size]\n`;
        flow += `    CheckExt -->|No| LoopNext[Next file]\n`;
        flow += `    GetStats --> CheckAge{Age > Expiry threshold?}\n`;
        flow += `    CheckAge -->|Yes| ModeCheck{Dry Run?}\n`;
        flow += `    CheckAge -->|No| LoopNext\n`;
        if (dry) {
          flow += `    ModeCheck -->|Yes| Log[Log files that would be deleted]\n`;
          flow += `    Log --> TrackStats[Increment deleted count & size]\n`;
        } else {
          flow += `    ModeCheck -->|No| Delete[Permanently delete file]\n`;
          flow += `    Delete --> TrackStats[Increment deleted count & size]\n`;
        }
        flow += `    TrackStats --> LoopNext\n`;
        flow += `    LoopNext --> WalkEnd{End of directory walk?}\n`;
        flow += `    WalkEnd -->|No| LoopFiles\n`;
        flow += `    WalkEnd -->|Yes| Report[Log reclaimed space stats]\n`;
        flow += `    Report --> ReportAlert{Purged > 0 and not Dry Run?}\n`;
        flow += `    ReportAlert -->|Yes| Alert[Dispatch cleanup status webhook alert]\n`;
        flow += `    ReportAlert -->|No| Finish([Finish cleanup sweep])\n`;
        flow += `    Alert --> Finish\n`;
      }
