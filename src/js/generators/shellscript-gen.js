  ...
  if (activeTab === 'flow') {
    ...
    function downloadScriptZip() {
      ...
      zip.generateAsync({ type: 'blob' }).then(blob => {
        ...
        a.click();
        showToast('⬇     Shell automation scripts downloaded successfully!');
      });
    }
    ...
    const tabExplanations = {'script': {'title': 'SRE Shell Automation Script', 'filename': 'backup_monitor.sh', 'why': 'Automates server cron tasks (log checks, backup runs, directory cleanups) using clean Bash structure.', 'when': 'Use for quick, lightweight automation scripts directly on Linux hosts.', 'where': 'Run on target host.', 'command': 'chmod +x backup_monitor.sh && ./backup_monitor.sh', 'practices': ['Enforce strict exit conditions (`set -euo pipefail`).', 'Quote variables to prevent syntax errors.', 'Log stdout and redirect stderr to track runs.'], 'ai_mlops': 'Runs backup scripts and triggers recovery actions for the **Self-Healing Infrastructure Platform**.', 'flow': '[Cron job scheduler] ➔ [Executes backup_monitor.sh] ➔ [Archive backup files]'}, 'readme': {'title': 'Shell Script Guide', 'filename': 'README.md', 'why': 'Outlines script flags, argument details, and cron job setup instructions.', 'when': 'Include in the script repository to help sysadmins maintain host automation scripts.', 'where': 'Save in the root of your scripting folder.', 'command': '# View in markdown reader', 'practices': ['Document crontab parameters.', 'List script flags.', 'Provide manual check steps.'], 'ai_mlops': 'Outlines cron tab schedulers settings for system check utilities.', 'flow': '[README.md Guide] ➔ [Guides safe execution steps]'}};
