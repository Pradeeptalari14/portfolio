// TruthTeller Evaluator SRE compiler logic
...
# Developer Studio configuration compiled for TruthTeller Evaluator
...
def init_rag_node():
    print("🚀 Initializing TruthTeller Evaluator agent retrieval sequence...")
...
`;
      ...
      compiledCode.primary_config = JSON.stringify({
        studioName: "TruthTeller Evaluator",
        version: "1.0.0",
        engine: "RAG-Agent",
        parameters: {
          endpoint: endpoint,
      ...
      compiledCode.primary_config = `-- Database indexing trigger setup for TruthTeller Evaluator
...
`;
      ...
      compiledCode.primary_config = `# SRE Config template for TruthTeller Evaluator
...
`;
    ...
    compiledCode.env_config = `# Environment settings for TruthTeller Evaluator
MODEL_ENDPOINT="${endpoint}"
...
# SRE validation checks helper for TruthTeller Evaluator
set -euo pipefail
...
echo "✅ Operational check complete for truthteller"
`;
...
`;
  ...
  window.SreCore.setupStudioTabs(
    ...
    (tabName) => {
      activeTab = tabName;
      let name = 'evaluator_config.yaml';
